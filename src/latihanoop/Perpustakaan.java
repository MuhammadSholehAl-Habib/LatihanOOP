...
public class Perpustakaan {
    ...
    //f.s: menampilkan daftar semua buku dengan memanggil buku.print()
        for(Integer daftar:daftarBuku.keySet()){
            daftarBuku.get(daftar).print();
        }
    }

    //lengkapi
    //input id buku, return objek bukunya
...
//        for (Peminjam p : daftarPeminjam.values()) {
//            p.print();
//        }
        for (Map.Entry<Integer, Peminjam> entry : daftarPeminjam.entrySet()) {
            Integer key = entry.getKey();
            Peminjam p = entry.getValue();
            p.print();
        }
    }
